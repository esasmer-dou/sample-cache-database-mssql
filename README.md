...
- `/actuator/health/liveness` reports process liveness. `/actuator/health/readiness` checks Redis, SQL Server and write-behind telemetry.
...
3. The owner reads at most `warm-max-rows` through the registered, bounded SQL Server JDBC loader.
4. Version-fenced hydration admits matching rows into Redis and refreshes projections.
5. Reconciliation removes rows that no longer match the policy from Redis without mutating SQL Server.
...
- Direct SQL Server writes appear on the next successful schedule. Use outbox/CDC when that lag is unacceptable.
...
|---|---|---|---|
| Health | `GET /actuator/health/readiness` | Runtime checks | Redis, SQL Server and write-behind readiness summary |
| Seed | `POST /api/demo/seed` | Bounded background job, batched Redis writes, SQL Server write-behind | Backpressure, SQL persistence, projection refresh, cross-pod job status |
...
| `GET /api/orders/archive` | `customerOrderArchiveSource(...)` | Generated bounded source query reads SQL Server directly | Does not mutate Redis | Cold/archive history path without controller SQL |
...
The generated source route compiles the same bounded `QuerySpec` through the SQL
Server dialect. The controller keeps the same response shape without owning SQL
text, a row mapper, or Redis hydration side effects.

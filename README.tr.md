...
- `/actuator/health/liveness` uygulama sürecinin çalıştığını, `/actuator/health/readiness` ise Redis, SQL Server ve write-behind durumunu gösterir.
...
3. Lease sahibi, kayıtlı ve sınırlı SQL Server JDBC loader üzerinden en fazla `warm-max-rows` kadar kayıt okur.
4. Sürüm kontrollü hydration, policy'nin kabul ettiği kayıtları Redis'e alır ve projection'ları yeniler.
5. Reconciliation, policy dışına çıkan kayıtları Redis'ten kaldırır; SQL Server'ı değiştirmez.
...
```powershell
Invoke-RestMethod "http://127.0.0.1:8092/api/warm/schedules"
```
...
- Doğrudan SQL Server'a yazılan kayıt bir sonraki başarılı çevrimde görünür. Bu gecikme kabul edilmiyorsa outbox/CDC kullanılmalıdır.
...
```bash
curl http://127.0.0.1:8092/actuator/health/readiness
```
...
```bash
curl "http://127.0.0.1:8092/api/warm/jobs/<jobId>"
```
...
|---|---|---|---|
| Sağlık | `GET /actuator/health/readiness` | Çalışma zamanı kontrolü | Redis, SQL Server ve write-behind hazırlık özeti |
| Veri üretme | `POST /api/demo/seed` | Sınırlı arka plan işi, toplu Redis yazma, SQL Server’a write-behind | Geri basınç, SQL kalıcılığı, projection yenileme ve pod’lar arası iş durumu |
...
| Sorgu yolu | Redis entity | Redis projection | SQL Server |
|---|---:|---:|---:|
...
| `GET /api/orders/{id}` | `linePreview` fetch preset ile `domain.<entity>().findById(...)` | Bu örnek endpoint’i Redis’te bulunamama durumunda SQL Server’a gitmez | Redis entity verisini okur, relation loader sınırlı satır sorgusunu Redis üzerinden yapar | Sık kullanılan sipariş detay ekranı |
| `GET /api/orders/archive` | `customerOrderArchiveSource(...)` | Üretilen sınırlı kaynak sorgusu SQL Server’ı doğrudan okur | Redis’i değiştirmez | Controller içinde SQL yazmadan arşiv/geçmiş okuması |
...
Üretilen kaynak route’u aynı sınırlı `QuerySpec` tanımını SQL Server dialect’i
